...
var Tab = (function(_Component) {
  ...
  function Tab(props) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (Tab.__proto__ || Object.getPrototypeOf(Tab)).call(this, props)
    );
    ...
    props.selection.register(props.tabFor, {
      ...
      setidfunc: this.props.setidfunc
    });
    return _this;
  }
  ...
  _createClass(Tab, [
    ...
    },
    {
      ...
      value: function update(e) {
        ...
        if (focus && this.props.selection.isSelected(this.props.tabFor)) {
          this.tabComponent.scrollIntoView({
            block: "nearest"
          });
    ...
    },
    {
      ...
      value: function handleFocus(event) {
        // console.log("★★-------------------------------------focused!!!!");
        const thisElm = document.activeElement;
        const prevElm = thisElm.parentElement.previousElementSibling;
        const nextElm = thisElm.parentElement.nextElementSibling;
        this.props.selection.prevTab = prevElm ? prevElm.id : "firstTab";
        this.props.selection.nextTab = nextElm ? nextElm.id : "lastTab";
        // console.log(prevElm);
        // console.log(`prev: ${this.props.selection.prevTab}`);
        // console.log(`next: ${this.props.selection.nextTab}`);
    ...
    },
    {
      ...
      value: function handleClick(event) {
        const option = {
          focus: true,
          setidfunc: this.props.setidfunc,
          delfunc: this.props.delfunc,
          favfunc: this.props.favfunc,
          tagfunc: this.props.tagfunc,
          storefunc: this.props.storefunc
        };
        this.props.selection.select(this.props.tabFor);
        ...
        if (this.props.onClick) {
          ...
          if (action === "action-delete") {
            this.props.selection.deleteItem(option);
          } else if (action === "action-star") {
            this.props.selection.favItem(option);
          } else if (action === "action-tag") {
            this.props.selection.tagItem(option);
          }
        }
      }
    },
    {
      ...
      value: function handleKeyDown(e) {
        ...
        if (e.keyCode === KeyCode.HOME) {
          this.props.selection.selectFirst(option);
        } else if (e.keyCode === KeyCode.END) {
          this.props.selection.selectLast(option);
        } else if (e.keyCode === KeyCode.LEFT_ARROW && !verticalOrientation) {
          this.props.selection.selectPrevious(option);
        } else if (e.keyCode === KeyCode.RIGHT_ARROW && !verticalOrientation) {
          this.props.selection.selectNext(option);
          ...
          e.preventDefault();
          this.props.selection.selectNext(option);
        } else if (e.keyCode === KeyCode.DELETE && verticalOrientation) {
          e.preventDefault();
          this.props.selection.deleteItem(option);
        } else if (
          e.keyCode === KeyCode.S &&
...
Tab.propTypes = {
  selection: _propTypes2.default.shape({
    ...
    favItem: _propTypes2.default.func.isRequired,
